...
func TestLossDSModifyCodeStep1(t *testing.T) {
	// File to modify
	filePath := "../../zk/stages/stage_sequence_execute_data_stream.go"
	...
	// Convert data to string for easier manipulation
	content := string(data)
	// Insert loss data logic
	blockToInsert := `
if request.BatchNumber >= 5 {
	return nil, nil
}`
	...
	inserted := false
	for i, line := range lines {
		if strings.Contains(line, "request := bundle.Request") {
			lines = append(lines[:i+1], append([]string{blockToInsert}, lines[i+1:]...)...)
			inserted = true
		}
	}
	require.True(t, inserted, "Expected the block to be inserted")
...
func TestLossDSStressStep2(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	...
	log.Info("Sleep to waiting...")
	time.Sleep(30 * time.Second)
	seqBatch, err := operations.GetBatchNumberBySeq()
	require.NoError(t, err)
	rpcBatch, err := operations.GetBatchNumber()
	require.NoError(t, err)

	log.Info(fmt.Sprintf("rpcBlock: %v, seqBlock: %v", rpcBatch, seqBatch))
	require.Less(t, rpcBatch+2, seqBatch)
}

func TestLossDSRecoverCodeStep3(t *testing.T) {
	filePath := "../../zk/stages/stage_sequence_execute_data_stream.go"
...
func TestLossDSCheckVerifyStep4(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
...
func TestRecoverModifyCodeStep1(t *testing.T) {
	// File to modify
	...
	// Insert loss data logic
	blockToInsert := `
zkevm.l1-sync-start-block: 353
zkevm.da-url: http://xlayer-da:8444`

	lines := strings.Split(content, "\n")
	inserted := false
	for i, line := range lines {
		if strings.Contains(line, "zkevm.l1-first-block: 353") {
			lines = append(lines[:i+1], append([]string{blockToInsert}, lines[i+1:]...)...)
			inserted = true
		}
	}
...
func TestRecoverRecoverCodeStep2(t *testing.T) {
	filePath := "../config/test.erigon.seq.config.yaml"
...
func TestRecoverCheckVerifyStep3(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
