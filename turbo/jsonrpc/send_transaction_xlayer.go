...
func (api *APIImpl) processBulk(bulk []txRequest) error {
	...
	if len(rlpTxs) > 0 {
		res, err := api.txPool.Add(ctx, &txPoolProto.AddRequest{RlpTxs: rlpTxs, DecodedTx: decodedTxs, RecoveredSender: senders})
		if err != nil {
			for i := range rlpTxs {
...
// returns txHash, decodedTx, sender
